...
class UARTWorker:
    ...
    TX_BYTES = 16_385   # 1 byte filter_id + 128 × 128 × 1
    RX_BYTES = 16_388   # 128 × 128 × 1 (16384) + 4 bytes cycle count
...
class FPGACoprocessorApp(ctk.CTk):
    ...
    def _build_sidebar(self, parent: ctk.CTkFrame) -> None:
        ...
        self._section_label(inner, "FILTER SELECTION")
    ...
    def _build_main(self, parent: ctk.CTkFrame) -> None:
        ...
        self._canvas_hw = ImageCanvas(
            dash, "SOFTWARE OUTPUT  ·  128 × 128  CONVOLVED", canvas_size=300
        )
    ...
    def _run_convolution(self) -> None:
        ...
        # Software sentinel bit: filter_id | 16
    ...
    def _on_result(self, raw: bytes) -> None:
        ...
        The FPGA sends 16388 bytes:
          - bytes  0..16383  → 128×128 convolved image pixels
          - bytes 16384..16387 → 32-bit cycle count (big-endian)
        """
        IMG_BYTES = 16_384  # 128 × 128

        # ── extract image data ────────────────────────────────────────
        img_data = raw[:IMG_BYTES]
        arr   = np.frombuffer(img_data, dtype=np.uint8).reshape((128, 128))
        img   = Image.fromarray(arr, mode="L")

        self._canvas_hw.display_image(img, badge="128×128 · L")
        self._result_img = img
        self._save_btn.configure(state="normal")

        # ── extract cycle count (last 4 bytes, little-endian) ────────────
        if len(raw) >= IMG_BYTES + 4:
            cycle_bytes = raw[IMG_BYTES:IMG_BYTES + 4]
            cycle_count = int.from_bytes(cycle_bytes, byteorder='little')
            self._log.log("OK",
