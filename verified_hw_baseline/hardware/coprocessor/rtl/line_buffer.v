// ============================================================
//  line_buffer.v
//  Author : Soumik Roy  (Noob_Duck)  ΓÇö Group 18
//
//  Purpose:
//    Holds 3 rows of a 128├ù128 grayscale image at a time.
//    Slides a 3├ù3 window across columns and exposes the 9
...
//    Repeats for rows 1ΓåÆ126  ΓåÆ  126 ├ù 126 = 15,876 output pixels.
//
//  Interface to surrounding modules:
//    ΓåÉ img_bram_in   : single-byte pixel read port
//    ΓåÆ conv_engine   : 9 ├ù 8-bit pixel window + window_valid
//    ΓåÆ img_bram_out  : output write address + write enable
//    ΓåÉ top_fsm       : start / done handshake
// ============================================================
...
module line_buffer #(
    ...
    // ΓöÇΓöÇ Handshake with top_fsm ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    input  wire        start,       // pulse: begin processing
    output reg         done,        // pulse: all valid pixels sent

    // ΓöÇΓöÇ img_bram_in read port ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    output reg  [13:0] bram_rd_addr,  // byte address  (128├ù128 = 16384 ΓåÆ 14 bits)
    input  wire [ 7:0] bram_rd_data,  // pixel byte returned next cycle

    // ΓöÇΓöÇ conv_engine pixel window ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    output reg  [ 7:0] p00, p01, p02,
    output reg  [ 7:0] p10, p11, p12,
    output reg  [ 7:0] p20, p21, p22,
    output reg         window_valid,  // 1 ΓåÆ conv_engine may latch this window

    // ΓöÇΓöÇ img_bram_out write port (we pass through pixel coords) ΓöÇΓöÇ
    output reg  [13:0] out_pixel_idx, // which output pixel this result maps to
    output reg         out_valid       // 1 ΓåÆ conv_engine result should be stored
);
...
// ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
//  3 ├ù 128 row registers   (prev / curr / next)
// ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
reg [7:0] row0_reg [0:127];   // prev  (oldest)
reg [7:0] row1_reg [0:127];   // curr  (middle)
reg [7:0] row2_reg [0:127];   // next  (newest)
...
localparam LOAD_ROW2   = 3'd3;   // fill row2_reg  (image row 2) ΓÇö first next row
localparam SLIDE       = 3'd4;   // slide col_ptr across 126 valid columns
localparam SHIFT_ROWS  = 3'd5;   // drop prev, shift currΓåÆprev, nextΓåÆcurr
...
//  Counters
// ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
reg  [6:0] load_col;   // 0..127  ΓÇö column index while loading a row
reg  [6:0] col_ptr;    // 0..125  ΓÇö leading-left column of the 3├ù3 window
                       //           window uses col_ptr, col_ptr+1, col_ptr+2
reg  [6:0] slide_row;  // 1..126  ΓÇö which image row is currently "curr" (row1)
reg  [6:0] next_row;   // image row index being loaded into row2_reg

// ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
//  BRAM read has 1-cycle latency ΓÇö we issue address one cycle
...
always @(posedge clk or negedge reset) begin
    if (!reset) begin
        ...
        // ΓöÇΓöÇ defaults (cleared every cycle unless re-asserted) ΓöÇΓöÇ
        done         <= 1'b0;
        window_valid <= 1'b0;
        out_valid    <= 1'b0;

        // ΓöÇΓöÇ absorb pending BRAM byte ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        // ΓöÇΓöÇ FSM ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        case (state)
            ...
            LOAD_ROW2: begin
                if (load_col == 7'd126) begin
                    // All 3 rows loaded ΓÇö begin sliding
                    load_col <= 7'd0;
            ...
            // ------------------------------------------------
            //  SLIDE : expose 3├ù3 window for each valid column
            //
            //  col_ptr  = 0..125  ΓåÆ window cols  col_ptr .. col_ptr+2
            ...
            // ------------------------------------------------
            SLIDE: begin
                // Drive the 3├ù3 pixel outputs (registered)
                ...
                if (col_ptr == 7'd125) begin
                    ...
                    if (slide_row == 7'd126) begin
                        // All 126 valid rows processed ΓÇö finished!
                        state <= DONE;
                    end
                    else begin
                        state <= SHIFT_ROWS;
            ...
            // ------------------------------------------------
            //  SHIFT_ROWS : row0 ΓåÉ row1 ΓåÉ row2
            //  (copy in a single cycle ΓÇö Verilog arrays allow
