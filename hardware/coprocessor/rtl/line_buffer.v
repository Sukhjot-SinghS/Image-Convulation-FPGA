...
// ─────────────────────────────────────────────────────────────
reg        bram_rd_pending;   // address has been issued, waiting for data
reg  [6:0] pending_col;       
reg  [1:0] pending_row_sel;   

// Pipeline stage to wait for BRAM data to arrive (1 cycle)
reg        bram_rd_valid;     // data is arriving THIS cycle
reg  [6:0] rd_col_q;          
reg  [1:0] rd_row_q;          
...
always @(posedge clk or negedge reset) begin
    if (!reset) begin
        ...
        // ── pipeline BRAM pending signals ────────────────────
        bram_rd_valid <= bram_rd_pending;
        rd_col_q      <= pending_col;
        rd_row_q      <= pending_row_sel;

        // ── defaults (cleared every cycle unless re-asserted) ──
        done            <= 1'b0;
        window_valid    <= 1'b0;
        out_valid       <= 1'b0;
        bram_rd_pending <= 1'b0; // default: no address being issued

        // ── absorb pending BRAM byte ──────────────────────────
        if (bram_rd_valid) begin
            case (rd_row_q)
                2'd0: row0_reg[rd_col_q] <= bram_rd_data;
                2'd1: row1_reg[rd_col_q] <= bram_rd_data;
                2'd2: row2_reg[rd_col_q] <= bram_rd_data;
                default: ;
            endcase
        end
