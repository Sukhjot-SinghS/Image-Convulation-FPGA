...
initial begin
    ...
    // cycle 3 after input
    @(posedge clk);
    check(out_valid == 0, "out_valid=0 at cycle+3 (too early)");

    // cycle 4 after input — NOW it should be valid
    @(posedge clk);
    check(out_valid == 1, "out_valid=1 at cycle+4 (exactly right)");

    // cycle 5 — should be gone (only 1 cycle pulse)
    @(posedge clk);
    check(out_valid == 0, "out_valid=0 at cycle+5 (pulse ended)");
    ...
    @(posedge clk);
    @(posedge clk);
    check(pixel_idx_out == 14'd42, "TEST10: pixel_idx_out=42 after 4 cycles");
    ...
    @(posedge clk);
    @(posedge clk);
    check(pixel_idx_out == 14'd1337, "TEST10: pixel_idx_out=1337 after 4 cycles");
    ...
    // wait for first result (4 cycles total from window 0)
    // already spent 5 cycles sending, so window 0 result
    // comes 4 cycles after its entry = cycle 4
    // we are currently at cycle 5, so result 0 came at cycle 4
    // let's step to capture each

    // result 0 should have appeared on cycle 4
    // we're now at cycle 5 — check result 1
    @(posedge clk);  // cycle 6 — result 1
