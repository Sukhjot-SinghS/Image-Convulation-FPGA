...
reg         norm_en;      // CHANGED: added norm_en control signal
...
conv_engine ce_inst (
    ...
    .k3(k3), .k4(k4), .k5(k5),
    .k6(k6), .k7(k7), .k8(k8),
    .norm_en      (norm_en),      // CHANGED: connected norm_en
    .window_valid (window_valid),
...
initial begin
    ...
    start           = 0;
    reset           = 1;
    norm_en         = 0;  // Initialise to Sobel/Sharpen mode
    ...
    if (read_val !== 8'd129) $display("  DEBUG: Test 1 first pixel was %d, expected 129", read_val);
    ...
    if (read_val !== 8'd130) $display("  DEBUG: Test 1 second pixel was %d, expected 130", read_val);
    check(read_val == 8'd130, "Second output pixel = 130");

    // 126th output pixel = row1,col126 = bram[254] = 254
    read_bram_out(14'd125, read_val);
    if (read_val !== 8'd254) $display("  DEBUG: Test 1 126th pixel was %d, expected 254", read_val);
    ...
    run_full_image;
    if (pixel_count != 15876) $display("  DEBUG: Test 6 pixel_count was %d, expected 15876", pixel_count);
