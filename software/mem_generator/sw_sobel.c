...
void sobel_filter(uint8_t input[HEIGHT][WIDTH], uint8_t output[HEIGHT-2][WIDTH-2]) {
    for (int r = 1; r < HEIGHT - 1; r++) {
        for (int c = 1; c < WIDTH - 1; c++) {
            ...
            int16_t total = (sumX < 0 ? -sumX : sumX) + (sumY < 0 ? -sumY : sumY);
...
int main() {
    ...
    static uint8_t image_in[HEIGHT][WIDTH] = {0};
    static uint8_t image_out[HEIGHT-2][WIDTH-2] = {0};
