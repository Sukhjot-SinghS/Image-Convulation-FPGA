...
//  TEST 2  — Pipeline delay is exactly 4 cycles
...
//  TEST 10 — pixel_idx_out arrives exactly 4 cycles after idx_in
//  TEST 11 — Pipeline throughput: 1 pixel per cycle
//  TEST 12 — out_valid follows window_valid with 4 cycle delay
// ============================================================
...
// ─────────────────────────────────────────────────────────────
//  Helper task — send one window and wait 4 cycles for result
// ─────────────────────────────────────────────────────────────
task send_window_and_wait;
    ...
    begin
        pixel_idx_in = idx;
        window_valid = 1;
        @(posedge clk);      // window sent
        window_valid = 0;
        pixel_idx_in = 0;
        @(posedge clk);      // stage 1
        @(posedge clk);      // stage 2
        @(posedge clk);      // stage 3 → output here
    end
endtask
...
initial begin
    ...
    //          Check out_valid appears exactly 4 cycles later
    // ─────────────────────────────────────────────
    $display("\n--- TEST 2: Pipeline delay = exactly 4 cycles ---");
    apply_reset;
    ...
    @(posedge clk);
    check(out_valid == 0, "out_valid=0 at cycle+1");

    // cycle 2 after input
    @(posedge clk);
    check(out_valid == 0, "out_valid=0 at cycle+2");

    // cycle 3 after input
    @(posedge clk);
    check(out_valid == 1, "out_valid=1 at cycle+3"); // ← was cycle+4
    ...
    k0 = 8'hFF; k1 = 8'h00; k2 = 8'h01;
    k3 = 8'hFE; k4 = 8'h00; k5 = 8'h02;
    k6 = 8'hFF; k7 = 8'h00; k8 = 8'h01;
    ...
    //          = -200 + (-400) + (-200) = -800
    //
    // WARNING: conv_engine inherently applies an abs_sum transformation!
    // |-800| = 800 -> 800 > 255 -> clamps to 255!
    send_window_and_wait(14'd0);
    check(out_valid == 1,    "TEST7: out_valid=1");
    check(pixel_out == 8'd255, "TEST7: negative abs result clamped to 255");
    ...
    // ─────────────────────────────────────────────
    $display("\n--- TEST 10: pixel_idx_out delayed by 4 cycles ---");
    apply_reset;
    set_kernel_identity;
    set_pixels(50,50,50, 50,50,50, 50,50,50);
    ...
    @(posedge clk);  // stage 1
    @(posedge clk);  // stage 2
    @(posedge clk);  // stage 3 → result here
    ...
    // Pipeline is 3 cycles
    // window 0 sent at cycle 1 → result at cycle 4
    // window 1 sent at cycle 2 → result at cycle 5
    // window 2 sent at cycle 3 → result at cycle 6
    // window 3 sent at cycle 4 → result at cycle 7
    // window 4 sent at cycle 5 → result at cycle 8
    // we are now at cycle 5 after sending window 4
    // need 3 more cycles to get window 1 result

    @(posedge clk);  // cycle 6 → window 0 result (p11=10) skip
    @(posedge clk);  // cycle 7 → window 1 result (p11=20)
    check(out_valid == 1,         "TEST11: result 1 valid");
    check(pixel_out == 8'd20,     "TEST11: result 1 = 20 (p11 of window 1)");
    check(pixel_idx_out == 14'd1, "TEST11: idx=1 correct");

    @(posedge clk);  // cycle 8 → window 2 result (p11=30)
    check(out_valid == 1,     "TEST11: result 2 valid");
    check(pixel_out == 8'd30, "TEST11: result 2 = 30");

    @(posedge clk);  // cycle 9 → window 3 result (p11=40)
    check(out_valid == 1,     "TEST11: result 3 valid");
    check(pixel_out == 8'd40, "TEST11: result 3 = 40");

    @(posedge clk);  // cycle 10 → window 4 result (p11=50)
