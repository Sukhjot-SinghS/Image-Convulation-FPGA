...
<img src="icon.png" alt="GIC Logo" width="220"/>

---

![Platform](https://img.shields.io/badge/Platform-Nexys%20A7%20Artix--7-blue?style=flat-square)
![ISA](https://img.shields.io/badge/ISA-RV32IM-green?style=flat-square)
![Clock](https://img.shields.io/badge/Clock-25%20MHz-orange?style=flat-square)
![Speedup](https://img.shields.io/badge/Speedup-~75×-red?style=flat-square)
![Course](https://img.shields.io/badge/Course-CS%20224-purple?style=flat-square)
...
## The Solution: Build the Hardware That Fits the Problem

Rather than squeezing performance from the general-purpose CPU, we designed a **dedicated hardware datapath** that does exactly one thing — but does it fast.
...
Vivado requires **absolute paths** to initialize Block RAMs at synthesis time. The repo ships an interactive helper that patches `hardware/cpu/rtl/memory.v` automatically:

```bash
python configure_paths.py
```

Press **Enter** to auto-detect your current directory, or type a custom path. The script updates both `$readmemh` calls in `memory.v` immediately.
...
The Vivado project directory is excluded from version control (rebuildable). Reconstruct it in one step:

1. Open **Vivado**
2. In the Tcl Console at the bottom, `cd` to the repo root
3. Run:
   ```tcl
   source rebuild_vivado_convolver.tcl
   ```

This recreates all IP blocks, source file links, constraint sets, and synthesis/implementation runs.
